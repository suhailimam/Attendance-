...
if(isset($_POST['addStudent'])){
  ...
  // Debugging: Check if images are received correctly
  if (empty($capturedImage1) || empty($capturedImage2)) {
      die("Captured images are empty. Please ensure that the images are being sent correctly.");
  }
  ...
  if (!file_put_contents($folderPath . '1.png', $imageData1)) {
      die("Failed to save image 1.png");
  }
  if (!file_put_contents($folderPath . '2.png', $imageData2)) {
      die("Failed to save image 2.png");
  }
  ...
  if($ret > 0){ 
      $message = "Student with the given Registration No: $registrationNumber Exists!";
  }
  else{
      ...
      VALUES('$firstName', '$lastName', '$email', '$registrationNumber', '$faculty', '$courseCode', '{$registrationNumber}_image1.png', '{$registrationNumber}_image2.png', '$dateRegistered')");
  ...
  <section class=main>
   ...
   <div class="table-container">
                ...
                <div class="table">
                    <table>
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>
                            <span><a href='edit.php?registrationNumber=" . $row["registrationNumber"] . "' class='ri-edit-line edit'></a></span>
                            <span><a href='deleteStudent.php?registrationNumber=" . $row["registrationNumber"] . "' onclick=\"return confirm('Are you sure you want to delete this student?');\"><i class='ri-delete-bin-line delete'></i></a></span>
                            </td>";
                            echo "</tr>";
                        }
                    } else {
    ...
    <form method="post">
        ...
        <div>
            <div>
                <input type="text" name="firstName" value="<?php echo isset($row['firstName']) ? $row['firstName'] : ''; ?>" placeholder="First Name">
                <input type="text" name="lastName" value="<?php echo isset($row['lastName']) ? $row['lastName'] : ''; ?>" placeholder="Last Name">
                <input type="email" name="email" value="<?php echo isset($row['email']) ? $row['email'] : ''; ?>" placeholder="Email Address">
                <input type="text" required name="registrationNumber" value="<?php echo isset($row['registrationNumber']) ? $row['registrationNumber'] : ''; ?>" placeholder="Registration Number">
